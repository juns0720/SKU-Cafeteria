...
> **실행 명세**: 각 단위 상세는 `01-phase-1-db.md` … `06-phase-d-photo.md`.
...
- [ ] **P1-T1**: V8 — `menus`에 `first_seen_at`/`last_seen_at` + 집계 캐시 5종(`avg_taste/amount/value/overall`, `review_count`) + 백필
...
- [ ] **P2-T5**: `MenuResponse` 확장 (`tier/isNew/firstSeenAt/avgTaste/Amount/Value/Overall`) — 기존 `averageRating/reviewCount` 호환 유지
